...
Phase 4.2: Download the Nuage VSP RPMs and Create a Yum Repository
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
The repository contents may change depending on the roles enabled in your deployment and may not contain all the packages listed in the following table.
...
::
   ...
   +----------------+----------------------------------------------+-------------------------------------------------------------------------------------------+
   | Nuage SR-IOV   | nuage-topology-collector                     |  nuage-openstack                                                                          |
   + ---------------+----------------------------------------------+-------------------------------------------------------------------------------------------+
   ...
   | VRS (OVRS)     +----------------------------------------------+-------------------------------------------------------------------------------------------+
   | Packages       | mstflint                                     | https://access.redhat.com/downloads/content/mstflint/4.11.0-5.el7/x86_64/fd431d51/package |
   +----------------+----------------------------------------------+-------------------------------------------------------------------------------------------+
...
2. Create the ComputeAvrs, ComputeOvrs, ComputeAvrsSingle and ComputeAvrsDual Nuage Compute roles:
   ::
...
For more information about the environment files, go to the `Sample Environment Files`_ and the *VSP User Guide*.
...
3. Assign roles to the Compute and Controller nodes, as described in the following sections.
Controller Role
''''''''''''''''
...
''''''''''''''''''''''''''''''''
    ...
    3. Modify the AVRS environment file in ``/home/stack/nuage-tripleo-heat-templates/environments/compute-avrs-environment.yaml``.
       ...
       a. For AVRS deployment, Virtual Accelerator requires information including which logical cores run the fast path, list of ports enabled in the fast path, additional fast path options, and so on to be set in `/etc/fast-path.env`.
          ...
          For the mapping between the parameters in environment files to the parameters in `fast-path.env`, see `Parameters Required for Nuage AVRS`_.
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    3. Modify the AVRS environment file in the ``/home/stack/nuage-tripleo-heat-templates/environments/compute-avrs-mutlirole-environment.yaml``.
       ...
       a. Virtual Accelerator requires information including which logical cores run the fast path, list of ports enabled in the fast path, additional fast path options, and so on to be set in `/etc/fast-path.env`.
          To find the mapping between the parameters in environment files to the parameters in `fast-path.env`, go to `Parameters Required for Nuage AVRS`_.
       b. Virtual Accelerator requires that the monkey_patch parameters be set in `nova.conf`. Use the following settings in `nova.conf`.
       ::
           ComputeAvrsExtraConfig:
               nova::config::nova_config:
                 DEFAULT/monkey_patch:
                 ...
                 DEFAULT/monkey_patch_modules:
                   value: nova.virt.libvirt.vif:openstack_6wind_extensions.queens.nova.virt.libvirt.vif.decorator
       c. Virtual Accelerator requires hugepages to be configured. To configure hugepages and enable VT-d:
       ::
           KernelArgs: "default_hugepagesz=1G hugepagesz=1G hugepages=64 iommu=pt intel_iommu=on isolcpus=1-7"
       .. Note::
          The kernel arguments are consumed by the another environment file that includes `/usr/share/openstack-tripleo-heat-templates/environments/host-config-and-reboot.yaml` in the deployment command.
...
'''''''''''''''''''''''''''''''
    ...
    3. As part of the Overcloud deployment, use the Mellanox firstboot template ``/home/stack/nuage-tripleo-heat-templates/firstboot/mellanox_fw_update.yaml``, which updates firmware on the Mellanox ConnectX-5 NIC interface.
...
'''''''''''''''''''''''''''
    ...
    1. When updating the Undercloud codebase, no additional changes are required.
    2. When modifying the overcloud-full image, use the script provided to update the image. No additional changes are required.
    3. Create a flavor and profile for ComputeSriov:
       Refer to https://access.redhat.com/documentation/en-us/red_hat_openstack_platform/13/html/director_installation_and_usage/chap-configuring_basic_overcloud_requirements_with_the_cli_tools#sect-Tagging_Nodes_into_Profiles for the detailed steps.
    ::
    ...
    4. Assign SR-IOV nodes with the appropriate ComputeSriov profile:
    ::
        openstack baremetal node set --property capabilities='profile:computesriov,boot_option:local' <node-uuid>
    5. To deploy the Overcloud, additional parameters and template files are required.
       a. Include the following parameter values in the Heat template *neutron-nuage-config.yaml*:
         ::
       ...
       b. Add this parameter value in the Heat template *nova-nuage-config.yaml*:
         ::
             NovaPCIPassthrough: "[{"devname":"eno2","physical_network":"physnet1"},{"devname":"eno3","physical_network":"physnet2"}]"
       c. Include  the *neutron-sriov.yaml* file in the Overcloud deployment command. For an example, go to `Sample Environment Files`_.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Nuage provides `single-nic-hw-offload network templates <../../nuage-tripleo-heat-templates/network/config/single-nic-hw-offload/>`_ for deploying the Overcloud Controller and ComputeOvrs by configuring a single Mellanox ConnectX-5 NIC for OVRS.
    By default, the Nuage-provided network templates support the following topology. You can modify these network templates to match yourpology.
    ...
    * computeovrs.yaml expects the ComputeOvrs nodes to have three interfaces, where the first interface is for provisioning, the second one is for all networks except the Tenant network with VLANs, and the Mellanox ConnectX-5 NIC interface (third interface) is only for the Tenant without VLANs.
    The following example shows the changes to the sample network template changes for a single Mellanox ConnectX-5 NIC:
    ::
            ...
            ...
                - type: sriov_pf
                  ...
                  - ip_netmask:
                      get_param: TenantIpSubnet
            ...
VF-LAG with VLANs for Mellanox ConnectX-5 NICs Without Control Plane Protection
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Nuage uses the default Linux bridge and Linux bonding.
    Nuage provides `bond-with-vlans network templates <../../nuage-tripleo-heat-templates/network/config/bond-with-vlans/>`_ to deploy the Overcloud Controller and ComputeOvrs by configuring Linux bonding with VLANs.
    By default, the Nuage-provided network template supports the following topology. You can modify the template to match your topology.
    * computeovrs.yaml expects the ComputeOvrs nodes to have five interfaces. The first interface is for provisioning. The second and third interfaces for Linux bonding with VLANs for all networks except the Tenant network. The rest are for OVRS using Mellanox ConnectX-5 NICs to configure Linux bonding with VLANs for the Tenant network.
    These are the changes in the sample network template for Linux bonding with VLANs for OVRS with Mellanox ConnectX-5 NICs:
    ::
            ...
            ...
              ...
              - type: vlan
                ...
                - ip_netmask:
                    get_param: TenantIpSubnet
            ...
VF LAG with VLANs for Mellanox ConnectX-5 NICs with Control Plane Protection
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Nuage uses the default Linux bridge and Linux bonding.
    Nuage provides `bond-with-vlans network templates <../../nuage-tripleo-heat-templates/network/config/bond-with-vlans/>`_ to deploy the Overcloud Controller and ComputeOvrs by configuring Linux bonding with VLANs.
    By default, the Nuage-provided network template supports the following topology. You can modify the template to match your topology.
            ...
            ...
              ...
              - type: vlan
                ...
                - ip_netmask:
                    get_param: TenantIpSubnet
            ...
    This example shows how to define the parameter inside network-environment.yaml and set this to appropriate values:
    ::
        TenantPortEthtoolOptions: "-L ${DEVICE} combined 60; -X ${DEVICE} equal 58; -U ${DEVICE} flow-type tcp4 src-port 6633 action 59 loc 1; -U ${DEVICE} flow-type tcp4 src-port 7406 action 59 loc 2; -U ${DEVICE} flow-type tcp4 src-port 39090 action 59 loc 3;-U ${DEVICE} flow-type tcp4 src-port 7407 action 59 loc 4"
.. Note::
...
Phase 9: Customize Nuage Templates with Custom Roles (Optional)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Your deployment may have customized roles other than the Nuage-defined roles for specific services.  You need to modify the appropriate environment files to define the services based on the customized roles. In this release, you can only customize OVRS-based roles.
If you create a new OVRS role called *CustomComputeOvrs*, change the following line in the `mellanox-environment.yaml <../../nuage-tripleo-heat-templates/environments/mellanox-environment.yaml>`_ file from:
::
    OS::TripleO::ComputeOvrs::NodeUserData: ../firstboot/mellanox_fw_update.yaml
to:
::
    OS::TripleO::CustomComputeOvrs::NodeUserData: ../firstboot/mellanox_fw_update.yaml
